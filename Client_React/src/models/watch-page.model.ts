...
export class WatchPage {
  ...
  getTypeByName(controllerName: string, symbolName: string): string {
    if (
      this._dataTypeCache[controllerName] === undefined ||
      this._dataTypeCache[controllerName][symbolName] === undefined
    ) {
      let list: IControllerSymbol[] = [];
      let path = this.findSymbolsByInput(
        controllerName,
        symbolName.replace(/\]+$/, ""),
        list,
      ); // remove the "]" at the end of an array
      if (list.length > 0) {
        this.cacheDataType(controllerName, symbolName, list[0].type);
      } else return ""; // nothing found for input symbol name
    }
  ...
  findSymbolsByInput(
    controllerName: string,
    symbolInputStr: string,
    list: IControllerSymbol[],
  ): string {
    list.length = 0;
    if (
      ...
      for (let idx in candidateSymbols) {
        ...
        if (match) {
          ...
          if (currentLevel == lastLevel) {
            ...
            if (
              candidateSymbols[idx].type.toLowerCase().startsWith("pointer to")
            ) {
              // dereference any pointer type encountered
              actualPath += "^";
              candidateSymbols =
                this.dataTypes[controllerName][
                  typeObj.baseType.toLocaleLowerCase()
                ].subItems;
            } else if (
              candidateSymbols[idx].type
                .toLowerCase()
                .startsWith("reference to")
            ) {
              ...
              if (typeObj.arrayDimension == 1) {
                ...
                newSymbolType = typeObj.name.replace(
                  /(?<=\[)\d+\.\.\d+,\s*/,
                  "",
                );
              }
  ...
  findPersistentSymbols(controllerName: string) {
    ...
    if (
      ...
      for (let symbolName in this.symbols[controllerName]) {
        this._findPersistentSymbolsRecursive(
          controllerName,
          this.symbols[controllerName][symbolName],
        );
      }
    }
  }
  ...
  _findPersistentSymbolsRecursive(
    controllerName: string,
    ...
    if (typeObj.arrayDimension > 0) {
      let newSymbolType =
        typeObj.arrayDimension == 1
          ? typeObj.baseType
          : typeObj.name.replace(/(?<=\[)\d+\.\.\d+,\s*/, ""); // This regexp removes the first dimension of array definition. ARRAY [a..b,c..d] becomes ARRAY [c..d]
      for (
        let i = typeObj.arrayInfo[0].startIndex;
        i < typeObj.arrayInfo[0].startIndex + typeObj.arrayInfo[0].length;
        i++
      ) {
        ...
        if (!hasPersistentData)
          // Try the first element in the array. If no persistent data, no need to continue.
          break;
      }
    } else if (typeObj.subItemCount > 0) {
      typeObj.subItems.forEach((subitem) => {
        hasPersistentData =
          this._findPersistentSymbolsRecursive(controllerName, {
